...
@interface AndyContentView ()<UIScrollViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource> {
    CGFloat   _oldOffSetX;
...
// 用于处理重用和内容的显示
@property (strong, nonatomic) AndyCollectionView *collectionView;
// collectionView的布局
...
// 所有的子控制器
@property (strong, nonatomic) NSMutableDictionary<NSString *, UIViewController<AndyScrollPageViewChildVcDelegate> *> *childVcsDic;
// 当前控制器
@property (strong, nonatomic) UIViewController<AndyScrollPageViewChildVcDelegate> *currentChildVc;
...
@implementation AndyContentView
...
- (void)commonInit {
    ...
    navi.automaticallyAdjustsScrollViewInsets = NO;
    ...
    if ([navi isKindOfClass:[UINavigationController class]]) {
        ...
        if (navi.interactivePopGestureRecognizer) {
            ...
            [self.collectionView setupScrollViewShouldBeginPanGestureHandler:^BOOL(AndyCollectionView *collectionView, UIPanGestureRecognizer *panGesture) {
...
- (void)receiveMemoryWarningHander:(NSNotificationCenter *)noti {
    ...
    __weak typeof(self) weakSelf = self;
    [_childVcsDic enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, UIViewController<AndyScrollPageViewChildVcDelegate> * _Nonnull childVc, BOOL * _Nonnull stop) {
        __strong typeof(self) strongSelf = weakSelf;
        if (strongSelf) {
            if (childVc != strongSelf.currentChildVc) {
                [self->_childVcsDic removeObjectForKey:key];
                [AndyContentView removeChildVc:childVc];
            }
        }
...
- (void)reload {
    ...
    [self.childVcsDic enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, UIViewController<AndyScrollPageViewChildVcDelegate> * _Nonnull childVc, BOOL * _Nonnull stop) {
        [AndyContentView removeChildVc:childVc];
        childVc = nil;
...
- (void)willAppearWithIndex:(NSInteger)index {
    UIViewController<AndyScrollPageViewChildVcDelegate> *controller = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", index]];
    if (controller) {
...
- (void)didAppearForStatistcsWithIndex:(NSInteger)index isTitleClicked:(BOOL)isTitleClicked
{
    UIViewController<AndyScrollPageViewChildVcDelegate> *controller = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", index]];
    if (controller) {
        if (_delegate && [_delegate respondsToSelector:@selector(scrollPageForStatisticsWithController:childViewControllDidAppear:forIndex:isTitleClicked:)]) {
            [_delegate scrollPageForStatisticsWithController:self.parentViewController childViewControllDidAppear:controller forIndex:index isTitleClicked:isTitleClicked];
        }
    }
}
...
- (void)setupChildVcForCell:(UICollectionViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(childViewController:forIndex:)]) {
        if (_currentChildVc == nil) {
            ...
            if (!_currentChildVc || ![_currentChildVc conformsToProtocol:@protocol(AndyScrollPageViewChildVcDelegate)]) {
                NSAssert(NO, @"子控制器必须遵守ZJScrollPageViewChildVcDelegate协议");
            }
            // 设置当前下标
            _currentChildVc.andy_currentIndex = indexPath.row;
    ...
    if (_currentChildVc != nil)
    {
        if (_currentChildVc.andy_scrollViewController != self.parentViewController) {
    ...
    {
        NSAssert(NO, @"发现 _currentChildVc = nil，请检查代码");
    }
}
...
//    NSLog(@"消失消失:current:---- %ld   old ----- %ld indexpathRow----%ld ", _currentIndex, _oldIndex, indexPath.row);
    ...
    if (!self.forbidTouchToAdjustPosition) {
        if (_currentIndex == indexPath.row) {// 没有滚动完成
            ...
            if (_needManageLifeCycle) {
                UIViewController<AndyScrollPageViewChildVcDelegate> *currentVc = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", _oldIndex]];
                // 开始出现
                [currentVc beginAppearanceTransition:YES animated:NO];
                
                UIViewController<AndyScrollPageViewChildVcDelegate> *oldVc = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", indexPath.row]];
                // 开始消失
            ...
            if (_oldIndex == indexPath.row) {
                ...
                [self didAppearForStatistcsWithIndex:_currentIndex isTitleClicked:self.forbidTouchToAdjustPosition];
            }
            else {
                // 滚动没有完成又快速的反向打开了另一页
                if (_needManageLifeCycle) {
...
- (AndyCollectionView *)collectionView {
    if (_collectionView == nil) {
        AndyCollectionView *collectionView = [[AndyCollectionView alloc] initWithFrame:self.bounds collectionViewLayout:self.collectionViewLayout];
...
- (NSMutableDictionary<NSString *,UIViewController<AndyScrollPageViewChildVcDelegate> *> *)childVcsDic {
    if (!_childVcsDic) {
