...
@interface AndySegmentStyle : NSObject

/** 是否显示底部的分割栏, 默认为NO */
@property (nonatomic, assign, getter=isShowBottomLineView) BOOL showBottomLineView;
/** 是否显示遮盖 默认为NO */
...
/** 左侧第一个标题距离左侧布局间隙 默认为15.0 titleMargin */
@property (assign, nonatomic) CGFloat firstTitleLeftMargin;
/** 标题的字体 默认为14 */
...
/** 有时scrollLine 会用图片 比如渐变图片代替，此时要设置 */
@property (nonatomic, strong) UIImage *scrollLineImage;

/** 当 scrollLineImage 有值时，此值才会生效, 如果 scrollLineImage设置了，但没有设置此处，则 scrollLineImageViewWith = scrollLine 默认行为宽度*/
@property (nonatomic, assign) CGFloat scrollLineImageViewWith;

/**  **/
@property (nonatomic, assign, getter=isNeedScrollLineDrawShadowColor) BOOL needScrollLineDrawShadowColor;

/** 顶部titles header的宽度，默认是起父控件 AndyScrollPageView 的宽度 */
@property (nonatomic, assign) CGFloat segmentViewWith;
